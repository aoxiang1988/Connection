				...
				case MotionEvent.ACTION_UP:
					if (mTouchState == TOUCH_STATE_SCROLLING) {
						...
						if (velocityX > SNAP_VELOCITY && mCurrentScreen > 0) {
							// 向右移动
//							MainActivity._inActivity.previous();
							snapToScreen(mCurrentScreen - 1, false);
						} else if (velocityX < -SNAP_VELOCITY
								&& mCurrentScreen < getChildCount() - 1) {
							// 向左移动
//							MainActivity._inActivity.next();
						snapToScreen(mCurrentScreen + 1, false);
						} else {
							snapToDestination();
						}
	...
	// 切换屏幕
	public void snapToScreen(int whichScreen, boolean isJump) {
		// 判断下一个屏幕是否有效，并纠正
		...
		if (getScrollX() != (whichScreen * getWidth())) {
			...
			if (t_mCurrentScreen != whichScreen) {
				...
				if (Math.abs(t_mCurrentScreen - whichScreen) == 1 && !isJump) {
					doOnScrollToScreen();
//					MainActivity._inActivity.ItemPlay(whichScreen);
				}
			}
			invalidate();
		}
	...
	public interface OnScrollToScreenListener {
		void operation(int currentScreen, int screenCount);
	}
	...
	interface OnCustomTouchListener {
		void operation(MotionEvent event);
	}
