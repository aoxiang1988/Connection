 ...
 * */
    public static Bitmap createReflectedImage(Bitmap originalImage) {
        ...
        // 创建总图片（原图片 + 倒影图片）
        Bitmap finalReflection = Bitmap.createBitmap(reflectionImage.getWidth(), (height + height / 2), Bitmap.Config.ARGB_8888);
        // 创建画布
        ...
        LinearGradient shader = new LinearGradient(0, originalImage.getHeight(), 0, finalReflection.getHeight(), 0x70ffffff,
                0x00ffffff, Shader.TileMode.MIRROR);
        shaderPaint.setShader(shader);
        shaderPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.DST_IN));
        //画布画出反转图片大小区域，然后把渐变效果加到其中，就出现了图片的倒影效果。
        canvas.drawRect(0, height, reflectionImage.getWidth(), finalReflection.getHeight(), shaderPaint);
