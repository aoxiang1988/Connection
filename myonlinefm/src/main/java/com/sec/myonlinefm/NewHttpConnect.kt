...
class NewHttpConnect {
    private val TAG: String? = "NewHttpConnect"
    var access_token: String? = null
    var token_type: String? = null
    var expires_in = 0
    var error: String? = null
    private val mQTUrl: String? = "https://open.staging.qingting.fm"

    /**
     * ** OAuth2.0授权 ***
     */
    fun getAccess_Token(context: Context?): String? {
        ...
        @SuppressLint("HardwareIds") val device_id = telephonyManager.deviceId
        val mToken_Url = "$mQTUrl/auth/v7/access?&grant_type=client_credentials"
        ...
        nameValuePairs.add(BasicNameValuePair("device_id", device_id))
    ...
    private fun getProperty_Url(category_id: Int): String? {
        return (mQTUrl + "/media/v7/categories/"
                + category_id
                + "?")
    }
...
//        nameValuePairs.add(new BasicNameValuePair("access_token", access_token));
        return httpGet(getProperty_Url(category_id))
    }
    ...
    private fun getAll_Station_Url(category_id: Int, curpage: Int, attr_id_1: Int, attr_id: Array<Int?>?): String? {
        ...
        mAll_Station_Url = if (attr_id_1 == 0) {
            if (attr_id == null) {
                mQTUrl +
                        "/media/v7/channelondemands?page=" +
                        curpage +
                        "&pagesize=" +
                        30 +
                        "&category_id=" +
